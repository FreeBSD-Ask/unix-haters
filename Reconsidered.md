...
在其他一些问题上，作者表现得不够好。“命令行为的一致性和可预测性，以及选项和参数的解释方式”听起来很不错，但第一部分是不可能的（不同命令必须有不同的行为，因为它们解决的问题不同），第二部分更多是抱怨 shell 的通配符扩展，而不是其他问题。抱歉，这个问题太有用了。如果你不喜欢 `rm *` 的工作方式，那就去启动图形文件管理器吧。
